...
func (u *UserStorage) RegisterUser(user *pb.RegisterUserRequest) (*pb.RegisterUserResponse, error) {
	...
	_, err = u.db.Exec(query, user.Email, hashedPassword, user.Role)
	if err != nil {
		return nil, err
	}
...
func (u *UserStorage) LoginUser(user *pb.LoginUserRequest) (*pb.LoginUserResponse, error) {
	query := `
	SELECT id, email, password_hash, role FROM users WHERE email = $1 AND deleted_at = 0
	`
	...
	fmt.Println("hash passwod >>>>>>>>>>>>>>>>>", res.PasswordHash, "\npassword >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>", user.PasswordHash)
...
func (u *UserStorage) UpdateUser(req *pb.UpdateUserRequest) (*pb.UpdateUserResponse, error) {
    ...
    if req.PasswordHash != "" && req.PasswordHash != "string" {
        hashedPassword, err := bcrypt.GenerateFromPassword([]byte(req.PasswordHash), bcrypt.DefaultCost)
        if err != nil {
            return nil, fmt.Errorf("failed to hash password: %v", err)
        }
        condition = append(condition, fmt.Sprintf("password_hash = $%d", len(args)+1))
    ...
    query += fmt.Sprintf(" WHERE id = $%d RETURNING id, email, password_hash, role", len(args)+1)
    args = append(args, req.Id)

    res := pb.UpdateUserResponse{}
    row := u.db.QueryRow(query, args...)

    err := row.Scan(&res.Id, &res.Email, &res.PasswordHash, &res.Role)
    if err != nil {
        return nil, err
    }
    return &res, nil
