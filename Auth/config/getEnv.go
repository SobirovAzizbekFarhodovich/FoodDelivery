...
func Load() Config {
	...
	config.GrpcUserPort = cast.ToString(getOrReturnDefaultValue("GRPC_USER_PORT", ":8072"))
	...
	config.PostgresHost = cast.ToString(getOrReturnDefaultValue("POSTGRES_HOST", "postgres_auth"))
	...
	config.RedisPort = cast.ToString(getOrReturnDefaultValue("REDIS_PORT", ":6380"))
